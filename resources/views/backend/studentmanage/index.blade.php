        ...
        <div id="kt_content_container" class="container-fluid">
            ...
            <div class="card mb-10">
                <div class="card-body d-flex align-items-center p-5 p-lg-8">
                    ...
                    <!--begin::Description-->
                    <div class="ms-6">
                        <p class="list-unstyled text-gray-600 fw-bold fs-6 p-0 m-0">Student Manage</p>
                    </div>
                    <!--end::Description-->
                </div>
            </div>
                    ...
                    <div class="card mb-5 mb-xl-8">
                        ...
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title align-items-start flex-column">
                                <span class="card-label fw-bolder fs-3 mb-1">All Student</span>
                                ...
                                <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3" id="dataTableExample1" class="data-table" style="width:100%;font-size:12px" >
                                    ...
                                    <thead class="text-center">
                                        <tr class="fw-bolder text-muted">
                                            ...
                                            <th class="min-w-140px">Notes</th>
                                             <th class="min-w-140px">Exam Attempts</th>
                                             <th class="min-w-140px">Booked Exams</th>
                                    ...
                                    @foreach($alldata as $key => $data)
                                        <tr>
                                            <td> {{ ++$key }} </td>
                                            <td>
                                                <a href="#" class="text-dark text-hover-primary d-block mb-1 fs-6">{{ $data->name }}</a>
                                            </td>
                                            <td>
                                                <a href="#" class="text-dark text-hover-primary d-block mb-1 fs-6">{{ $data->email }}</a>
                                            </td>
                                            <td>
                                                <a href="#" class="text-dark text-hover-primary d-block mb-1 fs-6">{{ $data->phone }}</a>
                                            </td>
                                             <td>
                                              <textarea onchange="notesupdate(this)" name="" id="{{ $data->id }}" class="form-control">{{ $data->notes }}</textarea>
                                            </td>
                                        @php
                                            $attemps=App\Models\ExamRequest::where('user_id',$data->id)->where('is_completed_from',0)->select(['main_exam_type'])->get();
                                            
                                            $booked_exam=App\Models\ExamRequest::where('user_id',$data->id)->where('is_completed_from',1)->select(['main_exam_type'])->where('is_deleted',1)->get();
                                        @endphp
                                             <td>
                                    @foreach($attemps as $examattem)
                                    <span class="badge badge-danger">{{$examattem->main_exam_type}}</span>
                                    @endforeach
                                 
                                    
                                            </td>
                                             <td>
                                    @foreach($booked_exam as $booksexam)
                                    <span class="badge badge-success">{{$booksexam->main_exam_type}}</span>
                                    @endforeach
                                            ...
                                            <td class="text-end">
                                                ...
                                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true" style="">
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a href="{{url('/admin/student/details/'.$data->id)}}" class="menu-link px-3">View</a>
                                                    </div>
                                                    <div class="menu-item px-3">
                                                    ...
                                                    <div class="menu-item px-3">
                                                        <a href="{{url('/admin/student/delete/'.$data->id)}}" class="menu-link px-3" data-kt-users-table-filter="delete_row">Delete</a>
                                                    </div>
                                            ...
                                            <td class="text-end">
                                                ...
                                                <a href="{{url('/admin/student/details/'.$data->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-eye blue"></i>
                                                </a>
                                                <a id="delete" href="{{url('/admin/student/delete/'.$data->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
...
<script>
    function notesupdate(el){
        var id=el.id;
        var val=el.value;
       
            $.ajax({
                 url: "{{  url('admin/update/studentnotes') }}",
                 type:"GET",
                 data: { 
                     "val": val,
                     "id": id,
