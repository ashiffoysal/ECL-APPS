                                ...
                                <table class="table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3" id="dataTableExample1" class="data-table" style="width:100%;font-size:14px" >
                                    ...
                                    <thead class="text-center">
                                        <tr class="fw-bolder text-muted">
                                            ...
                                            <th class="min-w-140px"> Candidate Number</th>
                                    ...
                                    <tbody class="text-center">
                                   	@php
                                   	$alldata=App\Models\ExamRequest::where('user_id',$data->id)->orderBy('id','DESC')->where('is_completed_from',1)->get();
                                   	@endphp
                                    @foreach($alldata as $key => $adata)
                                        <tr>
                                            <td> {{ ++$key }}</td>
                                            <td> {{ $adata->booking_id }}</td>
                                            <td> {{ $adata->main_exam_type }}</td>
                                            <td>
                                                @if($adata->Candidate_number !=null)
                                                 {{ $adata->Candidate_number }}
                                                @else
                                            ...
                                            <td> {{ $adata->created_at->format('d-M-Y') }} </td>
                                            <td> @if($adata->payment_option ==null) Not Yet @else{{  $data->payment_option  }} @endif</td>
                                            <td>
                                              @if($adata->status==0)
                                              <span class="btn-sm btn-warning">Pending</span>
                                              @elseif($adata->status==1)
                                              <span class="btn-sm btn-success">Approve</span>
                                              @elseif($adata->status==2)
                                              <span class="btn-sm btn-danger">Reject</span>
                                              @endif
                                            </td>
                                            <td class="text-end">
                                                @if($adata->status==1)
                                                <a href="{{url('admin/exambooking/maindreject/'.$adata->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-thumbs-up blue blue"></i>
                                                </a>
                                                @else
                                                  <a href="{{url('admin/exambooking/maindapprove/'.$adata->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-thumbs-up blue blue"></i>
                                                </a>
                                                @endif

                                                <a href="{{url('admin/exambooking/maindetails/'.$adata->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-eye blue blue"></i>

                                                </a>
                                                {{--
                                                <a href="{{url('admin/exambooking/details/'.$adata->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                <i class="fas fa-eye blue"></i>
                                                </a>
                                                --}}
                                                <a id="delete" href="#" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
