			...
			<div class="content-box">
				<ul class="course-info">
					...
					@if($data->mock_test=='mock_test_yes')
							...
							@if($data->marked_mocks=="I would like to marked mocks")
								@php
									$mockprice=$mockprice + 50;
								@endphp
							@endif
							
								
							@if($data->mock_test_paper_1 !=null)
								@php
									$mockprice=$mockprice + 20 ;
								@endphp
							@endif
							@if($data->mock_test_paper_2 !=null) 
								@php
									$mockprice=$mockprice + 20 ;
								@endphp
							@endif
							@if($data->mock_test_paper_3 !=null)
							 	@php
									$mockprice=$mockprice + 20 ;
								@endphp
							@endif
							@if($data->mock_test_paper_4 !=null) 
								@php
									$mockprice=$mockprice + 20 ;
								@endphp
							@endif
                ...
                <div class="info-column col-lg-6 col-md-12 col-sm-12">
                    <div class="inner-column">
                        ...
                        <div class="sec-title-two">
                            <div class="title" style="color: white;">Total Amount £ {{ $mainprice + $mockprice - $data->discount_amount}}</div>
                        </div>
                        <h6>Exam includes: </h6>
                        <ul>
	                		...
	                		@if($data->exam_information ==null)
								...
								@foreach(json_decode($data->exam_information) as $smainsub)
									...
									@endphp
									<li>{{$subjects->subject_name ?? ''}}: £ {{ $smainsub->fees ?? '' }}</li>

								@endforeach
							@endif
							<li>Discount Amount: £ {{$data->discount_amount}}</li>
                            
                        </ul>

                        <div class="row mt-5" style="color:#3ec7b0"> 
									<div class="col-md-4">
										<input type="text" name="cupon" id="cupon_code" class="form-control" placeholder="Use promo code">

										<span id="error_coupon" style="color:red;display:none">Please enter promo code</span>
										<span id="wrong_coupon" style="color:red;display:none">Coupon promo is Worng</span>
										<span id="success_coupon" style="color:green;display:none">Coupon promo add success</span>
										<span id="warning_coupon" style="color:red;display:none">Already used this promo code</span>
									</div>
									<div class="col-md-2">
										<button type="button" onclick="addcupon()" class="btn btn-warning">Apply</button>
									</div>
                        ...
                        <div class="contact-form">
                            ...
                            <div class="form-group">
                             ...
                             <div class="form-group" id="bank_new_payment" style="display:none;">
	                            <form method="post" action="{{ url('bank/payment') }}" enctype="multipart/form-data">
										@csrf
										<div class="row clearfix">
										...
										<input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
											...
											<!-- Form Group -->
											<input type="hidden" name="amount" value="{{ $mainprice + $mockprice }}">
						                    <input type="hidden" name="booking_id" value="{{ $data->booking_id }}">
							...
							<form action="{{ route('myonlinepayment.session') }}" method="POST">
	    						<input type="hidden" name="_token" value="{{csrf_token()}}">
								  <input type="hidden" name="amount" id="main_amount" value="{{ $mainprice + $mockprice - $data->discount_amount}}">
	                              <input type="hidden" name="myamount" id="myamount" value="{{ $mainprice + $mockprice}}">
								  ...
								  <button class="theme-btn btn-style-five" type="submit" id="checkout-live-button">Click here to pay with card ( £ {{ $mainprice + $mockprice - $data->discount_amount}} )</button>
							</form>
